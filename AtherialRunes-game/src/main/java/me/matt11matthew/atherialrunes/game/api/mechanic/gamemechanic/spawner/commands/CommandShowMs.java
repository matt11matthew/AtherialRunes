...
public class CommandShowMs extends AtherialCommand {
    public CommandShowMs(String command, String usage, String description, List<String> aliases) {
            ...
            if (Rank.isGM(gp.getName())) {
                if (args.length == 1) {
                    if (IntegerUtils.isInt(args[0])) {
                        ...
                        gp.msg(MessageType.CHAT, "&aShowing " + amount + " spawners");
                        gp.msg(MessageType.CHAT, "&7Radius: " + args[0].trim());
                        return true;
                    } else {
                        gp.msg(MessageType.CHAT, "&c/showms <radius>");
                        return true;
                    }
                } else {
                    gp.msg(MessageType.CHAT, "&c/showms <radius>");
